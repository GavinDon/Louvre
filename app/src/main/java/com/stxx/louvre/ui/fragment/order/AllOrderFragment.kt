...
class AllOrderFragment : BaseFragment() {
    ...
    private fun reqAllOrder() {
        RetrofitManager.create().getPersonalInfo()
                ...
                .compose(ProgressUtils.applyProgressBar(this.activity!!))
                .doOnNext {
                    if (0 != it.code) {
                ...
                .flatMap(Function<UserInfoBean, Observable<AllOrderBean>> {
                    return@Function RetrofitManager.create().getOrderStatus(it.member.userId + "")
                })
